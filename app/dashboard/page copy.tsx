...
import useSWR from "swr"; // Asegúrate de tener swr instalado
import {
  Video,
  FileText,
  Monitor,
...
import { useProjectStatus } from "./hooks/use-project-status"; // Importado de tu carpeta hooks
...
// --- SUBCOMPONENTE PARA VIGILAR CADA VIDEO INDEPENDIENTEMENTE ---
...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [profileData, videosResponse, projectsResponse, creditsResponse] = await Promise.all([
        ...
        getVideosByUser(authToken, backendUser.id),
        getListProjects(authToken),
        getCreditHistory(authToken, backendUser.id)
      ]);
  ...
  const handleRebuild = async (videoId: string, projectId: string) => {
    ...
    try {
      ...
      setVideos((prev) => prev.map((v) => (v.tavusVideoId === videoId ? { ...v, status: "queued" } : v)));
      ...
      <main className="mx-auto py-16">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {filters.map((filter) => (
              <button
                ...
                className={`rounded-full px-5 py-2 text-sm font-light transition-colors ${activeFilter === filter ? "bg-gray-700 text-white" : "bg-[#E2F2FE] text-[#080936] hover:bg-gray-300"}`}
              >
                {filter}
              </button>
            ))}
        ...
        {isLoading ? (
          <Card className="border-0 bg-white">
            <CardContent className="flex min-h-[400px] flex-col items-center justify-center p-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-pink-500"></div><p className="mt-4 text-gray-600">Syncing dashboard...</p></CardContent>
          </Card>
        ) : videosError ? (
          ...
          <CardContent className="flex min-h-[400px] flex-col items-center justify-center p-12 text-center">
            ...
            <Link href="/inputs"><Button size="lg" className="rounded-xl bg-[#6D58BB] px-6 py-6 text-lg font-semibold text-white hover:bg-gray-900">Start Creating Now <ArrowRight className="ml-2 h-5 w-5" /></Button></Link>
          </CardContent>
        ) : (
          <>
      ...
      <Dialog open={isVideoModalOpen} onOpenChange={setIsVideoModalOpen}>
        <DialogContent className="max-w-5xl p-0 overflow-hidden bg-black border-none ring-0">
          <div className="relative aspect-video w-full">
            <Button variant="ghost" size="icon" onClick={() => setIsVideoModalOpen(false)} className="absolute top-4 right-4 z-[110] bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-md"><X className="h-6 w-6" /></Button>
            <DialogHeader className="sr-only"><DialogTitle>Video Player</DialogTitle></DialogHeader>
            {selectedVideo && (selectedVideo.embed || (selectedVideo as any).metaData?.embed) ? (
              <div className="w-full h-full [&_iframe]:w-full [&_iframe]:h-full" dangerouslySetInnerHTML={{ __html: selectedVideo.embed || (selectedVideo as any).metaData?.embed }} />
            ) : (
