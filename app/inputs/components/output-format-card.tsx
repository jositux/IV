        ...
        <div className="bg-white rounded-[20px] border border-[#DADADA] px-4 py-4">
          ...
          <div className="grid grid-cols-5 gap-2">
            {VIDEO_LENGTHS.map((length) => {
              // LÓGICA DE MARCADO: Convertimos la etiqueta del loop a segundos para comparar con el estado
              const lengthInSeconds = length.toLowerCase().includes("m") 
                ? parseInt(length) * 60 
                : parseInt(length);
                
              const isSelected = Number(videoLength) === lengthInSeconds;
        ...
        <div className="bg-white rounded-[20px] border border-[#0B0F3A] px-4 py-4">
          ...
          <div className="flex gap-3 mb-4">
            ...
            >
              ...
              <span className="absolute inset-0 flex items-center justify-center text-[10px] text-gray-400 font-medium uppercase tracking-tighter">
                Bottom Left
              </span>
            </button>
            <button
            ...
            {["compact", "standard", "featured"].map((size) => {
              ...
              return (
                <div key={size} className="flex flex-col gap-1.5 w-full mb-2">
                  ...
                  >
                    {size}
                    <span className={`ml-2 text-xs font-normal ${isSelected ? "text-[#272830]/80" : "text-gray-400"}`}>
                      {dimensions[size as keyof typeof dimensions]}
                    </span>
                  </Button>
                  <div className="px-5">
                    {size === "compact" && <p className="text-[11px] text-gray-400 font-medium">Best for fast sharing and mobile previews</p>}
                    {size === "standard" && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[11px] font-bold text-[#0B0F3A]">Recommended</span>
                        <span className="text-[10px] text-amber-500">⭐</span>
                        <span className="text-[10px] text-gray-400 font-normal">(default)</span>
                      </div>
                    )}
                    {size === "featured" && <p className="text-[11px] text-gray-400 font-medium italic">High-fidelity output for professional use</p>}
                  </div>
                </div>
              );
            })}
