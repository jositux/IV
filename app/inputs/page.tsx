...
export default function VideoCreatorV2Page() {
  const {
    ...
    setPrimaryFocus,
    showSuccessModal,
    handleModalAccept, // Nuevos del hook
  } = useVideoCreator();

  const totalRequired = calculateTotalCredits();
  const isBalanceLoading = availableCredits === null;
  const hasEnoughCredits = isBalanceLoading
    ? true
    : availableCredits >= totalRequired;
      ...
      <div className=" p-4">
        ...
        <div className="py-8">
          <div className="text-center mb-12">
            <h1 className="mt-8 mb-4 text-5xl font-medium text-[#080936]">
        ...
        <Dialog open={showSuccessModal} onOpenChange={handleModalAccept}>
          <DialogContent className="bg-white rounded-[24px] p-8 border-none shadow-2xl max-w-sm flex flex-col items-center text-center">
            ...
            <DialogTitle className="text-2xl font-bold text-[#080936]">
              Project Started!
            </DialogTitle>
            <DialogDescription className="text-[#3E4462] mt-4 mb-6 text-base leading-relaxed">
              The process may take a few minutes. You can continue using the
              platform or start generating more projects in the meantime.
            </DialogDescription>
            <Button
