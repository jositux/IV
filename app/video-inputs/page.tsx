      ...
      <div className="sm:px-6 py-8 px-4">
        ...
        <Card className="p-4 sm:p-8 mb-8">
          ...
          {loadingReplicas ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {replicas.slice(0, 8).map((replica) => (
                  <div key={replica.replica_id} className="text-center">
                    <div
                      className={`relative rounded-xl border border-[#DADADA] overflow-hidden transition-all ${
                    ...
                    >
                      ...
                      <div
                        ...
                        className="relative aspect-[1/1] flex items-center justify-center cursor-pointer"
                      >
                        <div  className={`absolute p-2 ${
                        selectedReplica === replica.replica_id
                          ? "bg-[#0B0F3A] opacity-100"
                          : selectedReplica
                            ? "opacity-100"
                            : "hover:shadow-lg"
                      }`}>
           ...
           className="rounded-[8px] full-width"
          />
          </div>
                        ...
                        {replica.thumbnail_video_url && (
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-50 cursor-pointer">
                            <button
                              onClick={(e) => handlePlayVideo(replica.thumbnail_video_url, replica.replica_name, e)}
                              className="pointer-events-auto w-16 h-16 rounded-full bg-white/90 flex items-center justify-center shadow-lg hover:bg-white hover:scale-110 transition-all group"
                            >
                      ...
                      <div
                        onClick={() => setSelectedReplica(replica.replica_id)}
                        className={`py-3 px-2 flex items-center cursor-pointer gap-2 ${
                          selectedReplica === replica.replica_id ? "bg-[#0B0F3A] text-white" : "bg-white text-gray-900"
                        }`}
                      >
                        <p className="text-xl font-medium truncate px-4">{replica.replica_name}</p>
                        <div
                          className={`
  w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-colors
  ${
    selectedReplica === replica.replica_id
      ? "bg-[#6D58BB]" // Seleccionado: Morado
      : "bg-gray-300" // No seleccionado: Gris
  }
`}
                        >
                          <svg
                            className="w-3 h-3 text-white" // Icono siempre blanco
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
