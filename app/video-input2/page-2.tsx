...
export default function VideoCreatorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!topic && files.length === 0 && !generatedScript) {
        setError(
          "Please provide a topic, upload a file, or generate a prompt first"
        );
        setGenerating(false);
        return;
      }

      // Validate and create project if project name is provided
      let currentProjectId = projectId;
      if (projectName.trim() && !currentProjectId) {
        const newProjectId = await validateAndCreateProject();
        if (!newProjectId) {
          setGenerating(false);
          return;
        }
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 items-start">
                  ...
                  <Button
                    ...
                    disabled={checkingProject || !!projectId || projectName.trim().length < 4}
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <Button
              ...
              disabled={
                generating ||
                !selectedReplica ||
                (!topic && files.length === 0 && !generatedScript)
              }
              size="lg"
