...
export function ProfileMenu() {
  ...
  // Estado de carga inicial (esqueleto)
  if (isLoading) {
    return <div className="h-10 w-24 animate-pulse bg-white/10 rounded-lg" />
  }

  // Si hay un error 429 o de red, mostramos el login por seguridad
  if (isError && !user) {
    return (
      <Link href="/login">
        <Button variant="ghost" className="text-white text-xs opacity-70">
  ...
  return (
    <div className="flex items-center gap-4">
      {user ? (
        ...
        <Link href="/login">
          <Button variant="ghost" className="text-[16px] text-white hover:bg-white/10 transition-colors">
            Log In / Sign Up
          </Button>
        </Link>
      )}
